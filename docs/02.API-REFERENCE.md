...
Possible compile time enabled feature types:
 ...
 - JERRY_FEATURE_MEM_TRACK_ALLOCATION_SIZES - memory allocation size tracking (for heap snapshots)
 - JERRY_FEATURE_HEAP_SNAPSHOT - heap snapshots for memory profiling
...
## jerry_heap_snapshot_node_callback_t

**Summary**

Callback which is called at least once for every allocation in the JerryScript heap.

* `node` is a heap-unique identifier of the allocation
* `type` is the [jerry_heap_snapshot_node_type_t](#jerry_heap_snapshot_node_type_t) of the
  allocation
* `size` is the on-heap size of the allocation, in bytes (requires
  `JERRY_FEATURE_MEM_TRACK_ALLOCATION_SIZES`)
* `representation` is a JerryScript string containing a user-friendly representation of the
  allocation
* `representation_node` is the heap-unique identifier of the representation (if it exists on the
  heap, otherwise `NULL`)
* `user_data_p` is the pointer passed into
  [jerry_heap_snapshot_capture](#jerry_heap_snapshot_capture)
...
## jerry_heap_snapshot_edge_callback_t

**Summary**

Callback which is called at least once for every reference between allocations in the JerryScript
heap.

* `parent` is the heap-unique identifier of the referencing node
* `node` is the heap-unique identifier of the referenced node
* `type` is the [jerry_heap_snapshot_edge_type_t](#jerry_heap_snapshot_edge_type_t) of the
  edge
* `name` is a JerryScript string containing a user-friendly name of the
  allocation
* `name_node` is the heap-unique identifier of the name (if it exists on the
  heap, otherwise `NULL`)
* `user_data_p` is the pointer passed into
  [jerry_heap_snapshot_capture](#jerry_heap_snapshot_capture)
...
## jerry_heap_snapshot_node_id_t

A heap-unique identifier of a heap allocation. Not all heap allocations are `jerry_object_t`s,
refer to the `type` parameter of `jerry_heap_snapshot_node_callback_t`.

## jerry_heap_snapshot_node_type_t

Enum describing the type of a heap snapshot node. One of the following:

- JERRY_HEAP_SNAPSHOT_NODE_TYPE_HIDDEN - Engine-internal allocation
- JERRY_HEAP_SNAPSHOT_NODE_TYPE_ARRAY - Array
- JERRY_HEAP_SNAPSHOT_NODE_TYPE_STRING - String
- JERRY_HEAP_SNAPSHOT_NODE_TYPE_OBJECT - Object
- JERRY_HEAP_SNAPSHOT_NODE_TYPE_CODE - Bytecode
- JERRY_HEAP_SNAPSHOT_NODE_TYPE_CLOSURE - Closure
- JERRY_HEAP_SNAPSHOT_NODE_TYPE_NATIVE - Native reference (e.g. external C function)

## jerry_heap_snapshot_edge_type_t

Enum describing the type of a heap snapshot edge. One of the following:

- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_HIDDEN - Engine-internal reference
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_LEXENV - Lexical environment
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_PROTOTYPE - Object prototype
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_BIND - Functor returned by `func.bind()`
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_THIS - Object bound by `func.bind(obj, ...)`
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_BIND_ARGS - Argument bound by `func.bind(..., arg, ...)`
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_ELEMENTS - Contents of Typed array
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_PROPERTY - Property of Object
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_PROPERTY_NAME - Name of Property
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_PROPERTY_GET - Getter of Property
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_PROPERTY_SET - Setter of Property
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_PROMISE_RESULT - Result of Promise
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_PROMISE_FULFILL - Fulfillment handler of Promise
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_PROMISE_REJECT - Rejection handler of Promise
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_MAP_ELEMENT - Member of ES2015 Map built-in
- JERRY_HEAP_SNAPSHOT_EDGE_TYPE_SCOPE - Scope of function
...
Captures a snapshot of the JerryScript heap, useful for diagnosing JavaScript memory usage.
The heap is represented as a set of nodes (allocations) and directed edges (references) forming a
directed graph. Note that callbacks may be called more than once for a given node or edge.
...
void
jerry_heap_snapshot_capture (
                             jerry_heap_snapshot_node_callback_t node_cb,
                             jerry_heap_snapshot_edge_callback_t edge_cb,
                             void *user_data_p);
```

- `node_cb` - callback called for every node, see [jerry_heap_snapshot_node_callback_t](#jerry_heap_snapshot_node_callback_t)
- `edge_cb` - callback called for every edge, see [jerry_heap_snapshot_edge_callback_t](#jerry_heap_snapshot_edge_callback_t)
- `user_data_p` - user data pointer passed to `node_cb` and `edge_cb`
         ...
         void *user_data_p)
{
  ...
  {
    jerry_string_to_char_buffer (representation, (jerry_char_t *) repr_buf, sizeof (repr_buf));
  }

  printf ("Node ID: %" PRIdPTR " Type: %d Size: %zu Representation: %" PRIdPTR "=%s\n",
          node, type, size, representation_node, repr_buf);
} /* node_cb */
         ...
         void *user_data_p)
{
  ...
  {
    jerry_string_to_char_buffer (name, (jerry_char_t *) name_buf, sizeof (name_buf));
  }

  printf ("Edge %" PRIdPTR " -> %" PRIdPTR " Type: %d Name: %" PRIdPTR "=%s\n",
          parent, node, type, name_node, name_buf);
} /* edge_cb */
...
main (void)
{
  ...
  jerry_heap_snapshot_capture (node_cb, edge_cb, NULL);
...
- [jerry_heap_snapshot_node_callback_t](#jerry_heap_snapshot_node_callback_t)
- [jerry_heap_snapshot_edge_callback_t](#jerry_heap_snapshot_edge_callback_t)
- [jerry_heap_snapshot_node_id_t](#jerry_heap_snapshot_node_id_t)
