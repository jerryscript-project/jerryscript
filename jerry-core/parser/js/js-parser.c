                     ...
                     const ecma_parse_options_t *options_p) /**< additional configuration options */
{
    ...
    if (JERRY_LIKELY (!(bytecode_header_p->status_flags & CBC_CODE_FLAGS_STATIC_FUNCTION)))
    {
      ...
      if (CBC_SCRIPT_GET_TYPE (parent_script_p) != CBC_SCRIPT_GENERIC)
      {
        context.user_value = ((cbc_script_user_t *) parent_script_p)->user_value;
           ...
           && (context.options_p->options & ECMA_PARSE_HAS_USER_VALUE))
  {
  ...
  context.script_p->refs_and_type = (CBC_SCRIPT_REF_ONE | CBC_SCRIPT_GENERIC);

  if (context.user_value != ECMA_VALUE_EMPTY)
  {
    context.script_p->refs_and_type = (CBC_SCRIPT_REF_ONE | CBC_SCRIPT_USER_VALUE);

    if (ecma_is_value_object (context.user_value))
    {
      context.script_p->refs_and_type = (CBC_SCRIPT_REF_ONE | CBC_SCRIPT_USER_OBJECT);
    }
  }
