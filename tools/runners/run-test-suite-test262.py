...
def get_platform_cmd_prefix(is_python2: False):
...
def main(args):
    ...
    if args.es51:
        test262_harness_cmd = get_platform_cmd_prefix(True) + [ os.path.join(args.test262_harness_dir, 'tools/packaging/test262.py') ]
    else:
        test262_harness_cmd = get_platform_cmd_prefix(False) + [ os.path.join(args.test262_harness_dir, 'test262-harness.py') ]
