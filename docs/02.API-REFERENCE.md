...
## jerry_external_arraybuffer_free_callback_t

**Summary**

Free callback for external arraybuffers. See
[jerry_arraybuffer_set_external_free_callback](#jerry_arraybuffer_set_external_free_callback)
for more information.

**Prototype**

```c
typedef void (*jerry_external_arraybuffer_free_callback_t) (uint32_t length,
                                                            void *buffer_p,
                                                            void *user_p);
```

- `length` - external buffer length **in bytes**
- `buffer_p` - external buffer pointer
- `user_p` - pointer passed when the external arraybuffer is created
...
Sets the global callback which is called when an external arraybuffer is released.
It is recommended to set this function before the first external arraybuffer is created.

*Additional info*:
- When the callback is NULL, no function is called when an external arraybuffer is freed.
...
- `callback_p` - callback which is called when an external arraybuffer is freed.
...
static void
external_arraybuffer_free_callback (uint32_t length, /**< length of buffer */
                                    void *buffer, /**< buffer to free (if needed) */
                                    void *user_p) /**< user pointer */
{
  free (buffer);
  printf ("External arraybuffer is freed!\n");
}
...
main (void)
{
  ...
  jerry_arraybuffer_set_external_free_callback (external_arraybuffer_free_callback);
  
  const uint32_t buffer_size = 15;

  uint8_t *buffer_p = (uint8_t *) malloc (sizeof (uint8_t) * buffer_size);
  memset (buffer_p, 51 /* default value */, buffer_size);
  
  jerry_value_t external_arraybuffer = jerry_create_arraybuffer_external (buffer_size, buffer_p, NULL);
  
  /* The external_arraybuffer_free_callback is called eventually by the garbage collector. */
  jerry_release_value (external_arraybuffer);
  jerry_cleanup ();
  return 0;
}
```
...
- [jerry_external_arraybuffer_free_callback_t](#jerry_external_arraybuffer_free_callback_t)
- [jerry_create_arraybuffer_external](#jerry_create_arraybuffer_external)
- [jerry_create_shared_arraybuffer_external](#jerry_create_shared_arraybuffer_external)
...
After the object is not needed the garbage collector will call the callback which can be set by
[jerry_arraybuffer_set_external_free_callback](#jerry_arraybuffer_set_external_free_callback)
...
- `user_p` - user pointer passed to the callback when the arraybuffer is freed
- return value
...
User must pass a buffer pointer which is at least `size` big.
After the object is not needed the garbage collector will call the callback which can be set with
[jerry_arraybuffer_set_external_free_callback](#jerry_arraybuffer_set_external_free_callback)
so the user can release the buffer which was provided.
...
```c
{
  ...
  bool is_detachable = jerry_value_is_true (res);
