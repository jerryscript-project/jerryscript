...
def get_toolchain_from_arguments(arguments):
    if arguments.emscripten_simulated_jerry_api == 'ON':
        if arguments.toolchain:
            print('Cannot use --toolchain when using --emscripten-simulated-jerry-api', file=sys.stderr)
            sys.exit(-1)
...
def get_arguments():
    ...
    parser.add_argument('--doctests', action='store_const', const='ON', default='OFF',
                        help='build doctests')
    parser.add_argument('--emscripten-simulated-jerry-api', metavar='X', choices=['ON', 'OFF'], default='OFF',
                        type=str.upper, help='build simulated JerryScript implementation'
...
def generate_build_options(arguments):
    ...
    build_options.append('-DEMSCRIPTEN_SIMULATED_JERRY_API=%s' % arguments.emscripten_simulated_jerry_api)
