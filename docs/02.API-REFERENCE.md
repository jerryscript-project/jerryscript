...
- The engine considers errors thrown by external functions as never caught. The
  application can maintain a status flag to ignore the next call of the callback if necessary.
...
*Additional info*:
- Since internally the pointer to a manager's context data item is linked to the next such pointer in a linked
...
*Additional info*:
- For non-object parameters `JERRY_OBJECT_TYPE_NONE` is returned.
- the returned type can be checked for more detailed type information in the following cases:
...
*Additional info*:
- For non-function parameters `JERRY_FUNCTION_TYPE_NONE` is returned.
...
*Additional info*:
- For non-iterator parameters `JERRY_ITERATOR_TYPE_NONE` is returned.
...
*Additional info*:
- The difference from [jerry_get_string_size](#jerry_get_string_size) is that it returns with utf-8 string size
...
*Additional info*:
- Does not put '\0' to the end of string, the return value identifies the number of valid bytes in the output buffer.
...
*Additional info*:
- Every created API value must be freed with [jerry_release_value](#jerry_release_value) when it
  is no longer needed.
...
*Additional info*:
- The undefined value is a fixed constant.
  Its value can be copied any number of times without calling [jerry_acquire_value] (#jerry_acquire_value), and freeing it with [jerry_release_value](#jerry_release_value) is optional.
...
*Additional info*:
- `out_native_pointer_p` can be NULL, and it means the caller doesn't want to get the native_pointer.
...
- If possible do not store API values in native pointers, rather
  check [jerry_set_internal_property](#jerry_set_internal_property).
...
*Additional info*:
- Values obtained in `foreach_p` must be retained using [jerry_acquire_value](#jerry_acquire_value).
...
*Additional info*:
- Calling this method is safe in any time. It can be called even before engine initialization.
...
*Additional info*:
- If the ArrayBuffer has been created with `jerry_create_arraybuffer_external` the optional 
  free callback is called on a successful detach operation
