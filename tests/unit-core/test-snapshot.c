...
static void test_snapshot_with_user (void)
{
      ...
      && jerry_is_feature_enabled (JERRY_FEATURE_SNAPSHOT_EXEC))
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      {
        parse_options.user_value = jerry_create_number (-3.5);
      }
      else
      {
        parse_options.user_value = jerry_create_string ((const jerry_char_t *) "AnyString...");
      }

      result = jerry_exec_snapshot (snapshot_buffer, snapshot_size, 0, 0, &parse_options);
