...
typedef struct
{
  ecma_json_token_type_t type;
  ecma_char_t *current_p;
  ecma_char_t *end_p;
  union
  {
    ecma_char_t *string_start_p;
 ...
 */
static bool
ecma_builtin_json_check_id (ecma_char_t *string_p, /**< start position */
                            const char *id_p) /**< string identifier */
{
  /*
   * String comparison must not depend on ecma_char_t definition.
   */
...
{
  ecma_char_t *current_p = token_p->current_p;
  ecma_char_t *write_p = current_p;

  token_p->u.string_start_p = current_p;
...
ecma_builtin_json_parse_number (ecma_json_token_t *token_p) /**< token argument */
{
  ...
  ecma_char_t saved_char = *current_p;
  *current_p = '\0';

  token_p->type = number_token;
  token_p->u.number = ecma_zt_string_to_number (start_p);

  *current_p = saved_char;
...
ecma_builtin_json_parse_value (ecma_json_token_t *token_p) /**< token argument */
{
  ...
  switch (token_p->type)
  {
    ...
    case string_token:
    {
      return ecma_make_string_value (ecma_new_ecma_string (token_p->u.string_start_p));
    }
    case null_token:
    {
    ...
    case left_brace_token:
    {
      ...
      while (true)
      {
        ...
        ecma_char_t *string_start_p = token_p->u.string_start_p;
        ecma_builtin_json_parse_next_token (token_p);
        ...
        ecma_string_t *name_p = ecma_new_ecma_string (string_start_p);
                        ...
                        ecma_string_t *name_p) /** < property name */
{
  ...
  if (ecma_is_value_object (value))
  {
    ...
    if (no_properties > 0)
    {
      size_t buffer_size = sizeof (ecma_string_t*) * (no_properties);
      ecma_string_t **property_names_p = (ecma_string_t**) mem_heap_alloc_block (buffer_size,
                                                                                 MEM_HEAP_ALLOC_SHORT_TERM);

      if (property_names_p == NULL)
      {
      ...
      {
        completion_value = ecma_builtin_json_walk (reviver_p,
                                                   object_p,
        ...
        ecma_value_t value = ecma_get_completion_value_value (completion_value);
      ...
      mem_heap_free_block (property_names_p);
    }
  }
                         ...
                         ecma_value_t arg2) /**< reviver argument */
{
  ...
  ecma_char_t *str_start_p = (ecma_char_t*) mem_heap_alloc_block (buffer_size,
                                                                  MEM_HEAP_ALLOC_SHORT_TERM);
  ...
  ecma_string_to_zt_string (string_p, str_start_p, (ssize_t) buffer_size);
  ...
  else
  {
    ...
    {
      ecma_object_t *object_p = ecma_op_create_object_object_noarg ();
      ecma_string_t *name_p = ecma_get_magic_string (ECMA_MAGIC_STRING__EMPTY);
  ...
  mem_heap_free_block (str_start_p);
                             ...
                             ecma_value_t arg3)  /**< space */
{
  ...
  /* 2. */
  context_p.indent_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING__EMPTY);
  ...
  if (ecma_is_value_object (arg2))
  {
    ...
    /* 4.b */
    else if (ecma_object_get_class_name (obj_p) == ECMA_MAGIC_STRING_ARRAY_UL)
    {
      ecma_string_t *length_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_LENGTH);
      ...
      while ((index < array_length) && ecma_is_completion_value_empty (ret_value))
      {
        ...
        else if (ecma_is_value_object (value))
        {
          ...
          ecma_magic_string_id_t class_name = ecma_object_get_class_name (obj_val_p);

          /* 4.b.ii.4.a */
          if (class_name == ECMA_MAGIC_STRING_NUMBER_UL
  ...
  if (ecma_is_value_object (arg3))
  {
    ...
    ecma_magic_string_id_t class_name = ecma_object_get_class_name (obj_p);
    ...
    /* 5.b */
    else if (class_name == ECMA_MAGIC_STRING_STRING_UL)
    {
      ECMA_TRY_CATCH (val,
  ...
  if (ecma_is_value_number (space))
  {
    ...
    if (space < 1)
    {
      context_p.gap_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING__EMPTY);
    }
    else
    {
      MEM_DEFINE_LOCAL_ARRAY (space_buff, space + 1, char);
      ...
      context_p.gap_str_p = ecma_new_ecma_string ((ecma_char_t *) space_buff);
  ...
  else if (ecma_is_value_string (space))
  {
    ...
    int32_t num_of_chars = ecma_string_get_length (space_str_p);
    ...
    {
      int32_t string_len = ecma_string_get_length (space_str_p);
      ...
      ssize_t bytes_copied = ecma_string_to_zt_string (space_str_p,
                                                       zt_string_buff,
                                                       (ssize_t) string_buf_size);
      JERRY_ASSERT (bytes_copied > 0);

      /* Buffer for the first 10 characters. */
      MEM_DEFINE_LOCAL_ARRAY (space_buff, 11, ecma_char_t);
  ...
  ecma_string_t *empty_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING__EMPTY);
...
{
  /* 1. */
  ecma_string_t *quote_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_DOUBLE_QUOTE_CHAR);
  ecma_string_t *product_str_p = ecma_copy_or_ref_ecma_string (quote_str_p);
  ecma_string_t *tmp_str_p;

  int32_t string_len = ecma_string_get_length (string_p);

  MEM_DEFINE_LOCAL_ARRAY (zt_string_buff, string_len + 1, ecma_char_t);

  size_t string_buf_size = (size_t) (string_len + 1) * sizeof (ecma_char_t);
  ssize_t bytes_copied = ecma_string_to_zt_string (string_p,
                                                   zt_string_buff,
                                                   (ssize_t) string_buf_size);
  JERRY_ASSERT (bytes_copied > 0);
  ...
  {
    ecma_char_t c = zt_string_buff[i];

    /* 2.a */
    if (c == '\\' || c == '\"')
    {
      ecma_string_t *backslash_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_BACKSLASH_CHAR);
      ...
      /* 2.a.ii */
      ecma_char_t zt_c[2] = {c, '\0'};
      ecma_string_t *c_str_p = ecma_new_ecma_string (zt_c);
    ...
    else if (c == '\b' || c == '\f' || c == '\n' || c == '\r' || c == '\t')
    {
      ...
      /* 2.b.iii */
      ecma_char_t zt_abbrev[2] = {abbrev, '\0'};
      ecma_string_t *abbrev_str_p = ecma_new_ecma_string (zt_abbrev);
    ...
    else if (c < 32)
    {
      ...
      /* 2.c.ii */
      ecma_char_t zt_u_ch[2] = {'u', '\0'};
      ecma_string_t *u_ch_str_p = ecma_new_ecma_string (zt_u_ch);
                       ...
                       stringify_context_t *context_p) /**< context*/
{
  ...
  /* 1. */
  ecma_completion_value_t get_comp_val = ecma_op_object_get (holder_p, key_p);

  if (!ecma_is_completion_value_normal (get_comp_val))
  {
    return get_comp_val;
  }

  value = ecma_copy_value (ecma_get_completion_value_value (get_comp_val), true);
  ecma_free_completion_value (get_comp_val);

  /* 2. */
  if (ecma_is_value_object (value))
  {
    ecma_object_t *value_obj_p = ecma_get_object_from_value (value);
    ecma_string_t *to_json_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_TO_JSON_UL);
    ...
    if (ecma_op_is_callable (toJSON))
    {
      ...
      ECMA_TRY_CATCH (func_ret_val,
                      ecma_op_function_call (toJSON_obj_p, value, call_args, 1),
                      ret_value);

      ecma_free_value (value, true);
      value = ecma_copy_value (func_ret_val, true);
  ...
  /* 4. */
  if (ecma_is_value_object (value) && ecma_is_completion_value_empty (ret_value))
  {
    ...
    if (class_name == ECMA_MAGIC_STRING_NUMBER_UL)
    {
      ...
      ecma_free_value (value, true);
      value = ecma_copy_value (val, true);
    ...
    /* 4.c */
    else if (class_name == ECMA_MAGIC_STRING_BOOLEAN_UL)
    {
      ECMA_TRY_CATCH (val,
                      ecma_op_to_primitive (value, ECMA_PREFERRED_TYPE_NO),
                      ret_value);
  ...
  {
    ecma_completion_value_t str_comp_val = ecma_op_to_string (value);
    ecma_free_value (value, true);
  ...
  if (ecma_is_value_string (value))
  {
    ...
    ecma_completion_value_t ret_comp = ecma_builtin_json_quote (value_str_p);
    ecma_free_value (value, true);
  ...
  {
    ecma_number_t num_value_p = *ecma_get_number_from_value (value);
    ...
    /* 9.b */
    ecma_string_t *null_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_NULL);
    return ecma_make_normal_completion_value (ecma_make_string_value (null_str_p));
  }
  ...
  if (ecma_is_value_object (value) && !ecma_op_is_callable (value))
  {
    ...
    /* 10.a */
    if (class_name == ECMA_MAGIC_STRING_ARRAY_UL)
    {
      ECMA_TRY_CATCH (val,
                          ...
                          stringify_context_t *context_p) /**< context*/
{
       ...
       key_p++)
  {
    ...
    {
      ecma_string_t *colon_p = ecma_get_magic_string (ECMA_MAGIC_STRING_COLON_CHAR);
      ...
      if (!is_gap_empty)
      {
        ecma_string_t *space_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_SPACE_CHAR);
  ...
  {
    ecma_string_t *left_brace_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_LEFT_BRACE_CHAR);
    ecma_string_t *right_brace_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_RIGHT_BRACE_CHAR);
                         ...
                         stringify_context_t *context_p) /**< context*/
{
       ...
       index++)
  {
    ...
    if (ecma_is_value_undefined (str_val))
    {
      list_append (&partial, ecma_get_magic_string (ECMA_MAGIC_STRING_NULL));
    }
    /* 8.c */
    else
    {
  ...
  if (ecma_is_completion_value_empty (ret_value))
  {
    ...
    if (list_is_empty (&partial))
    {
      ecma_string_t *left_square_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_LEFT_SQUARE_CHAR);
      ecma_string_t *right_square_str_p = ecma_get_magic_string (ECMA_MAGIC_STRING_RIGHT_SQUARE_CHAR);
