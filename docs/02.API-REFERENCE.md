...
- [jerry_load_function_snapshot](#jerry_load_function_snapshot)
...
- `options_p` - additional loading options, can be NULL if not used. Only `JERRY_PARSE_HAS_RESOURCE`
                and `ECMA_PARSE_HAS_USER_VALUE` options are supported.
- return value
  - result of bytecode, if run was successful.
