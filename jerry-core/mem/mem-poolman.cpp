...
mem_pools_collect_empty (void)
{
   ...
   *    first chunk in the same pool, and move those chunks, for which the condition is true, to separate temporary list.
   *
   ...
   */
  {
...

    {
       ...
       */
      for (mem_pool_chunk_t *pool_list_iter_p = first_chunks_list_p;
           pool_list_iter_p != NULL;
           pool_list_iter_p = MEM_CP_GET_POINTER (mem_pool_chunk_t,
                                                  pool_list_iter_p->u.pool_gc.next_first_cp))
      {
        JERRY_ASSERT (!pool_list_iter_p->u.pool_gc.traversal_check_flag);

        pool_list_iter_p->u.pool_gc.traversal_check_flag = true;
      }
    }

    {
           ...
           non_first_chunks_iter_p = next_p)
      {
        ...
        mem_pool_chunk_t *pool_start_p = (mem_pool_chunk_t *) mem_heap_get_chunked_block_start (non_first_chunks_iter_p);
        ...
        for (mem_pool_chunk_t *pool_list_iter_p = first_chunks_list_p;
             ...
             pool_list_iter_p = MEM_CP_GET_POINTER (mem_pool_chunk_t,
                                                    pool_list_iter_p->u.pool_gc.next_first_cp))
        {
          if (pool_start_p == pool_list_iter_p)
          {
            is_occured = true;
            break;
          }
   ...
   */
  for (mem_pool_chunk_t *pool_list_iter_p = first_chunks_list_p, *next_p;
       pool_list_iter_p != NULL;
       pool_list_iter_p = next_p)
  {
    ...
    JERRY_ASSERT (pool_list_iter_p->u.pool_gc.hint_magic_num == hint_magic_num_value);
    JERRY_ASSERT (pool_list_iter_p->u.pool_gc.traversal_check_flag);
    JERRY_ASSERT (pool_list_iter_p->u.pool_gc.free_chunks_num <= MEM_POOL_CHUNKS_NUMBER);
