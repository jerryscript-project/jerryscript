...
Structure that holds a single user context item, identified by its manager. It is intended to serve as the base for
larger structures that need to be allocated at runtime. Thus, the members of the structure are to be treated as opaque,
because JerryScript will take care of initializing them.
...
- [jerry_get_context_data](#jerry_get_context_data)
...
Structure that defines how a user context item will be initialized and deinitialized.
...
**N.B.:** Since internally the pointer to a manager's context data item is linked to the next such pointer in a linked
list, it is inadvisable to invoke too many different managers, because doing so will increase the time it takes to
retrieve a manager's context data item, degrading performance. For example, try to keep the number of managers below
five.
...
- return value: the item created by `manager_p` when `jerry_get_context_data ()` was first called, or a new item created
  ...
  freed using the `deinit_cb` callback provided by `manager_p` when the context will be destroyed.
...
/* Define how context items will be created */
static jerry_context_data_header_t *
my_context_data_new (void)
{
  my_context_data_t *my_data;

  /* Allocate and initialize my_data. JerryScript will initialize the base. */

  return ((jerry_context_data_header_t *) my_data);
}

/* Define how context items will be destroyed */
static void
my_context_data_free (jerry_context_data_header_t *item)
{
  my_context_data_t *my_data = ((my_context_data_t *) item);

  /* perform any necessary cleanup on my_data and then free my_data */
}
 ...
 * context, creating such an item if one was not previously created */
void someplace_in_the_code (void)
{
  my_context_data_t *my_data = (my_context_data_t *) jerry_get_context_data (&my_manager);
  /* Perform useful things using the data found in my_data, but do not touch my_data->base. */
}
```
