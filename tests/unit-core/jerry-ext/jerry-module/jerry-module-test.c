...
const char eval_string[] = "require ('my_custom_module');";
...
main (int argc, char **argv)
{
  (void) argc;
  (void) argv;
  jerry_value_t js_global = 0, js_function = 0, js_property_name = 0, js_eval_result = 0;
  ...
  js_eval_result = jerry_eval ((const jerry_char_t *) eval_string, sizeof (eval_string) - 1, true);

  TEST_ASSERT (jerry_get_number_value (js_eval_result) == 42);
