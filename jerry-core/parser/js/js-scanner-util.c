 ...
 */
void
scanner_release_private_fields (scanner_class_private_member_t *member_p) /**< private fields */
{
  while (member_p != NULL)
  {
                                ...
                                uint8_t opts) /**< options */
{
  ...
  uint8_t search_flag = (opts & SCANNER_PRIVATE_FIELD_PROPERTY) ? SCANNER_PRIVATE_FIELD_PROPERTY_GETTER_SETTER : opts;

  while (iter != NULL)
  {
    if (lexer_compare_identifiers (context_p, &context_p->token.lit_location, &iter->loc)
        && (iter->flags & search_flag))
    {
      scanner_raise_error (context_p);
    }
