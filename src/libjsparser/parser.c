  ...
  if (tok.type == TOK_QUERY)
    {
      ...
      DUMP_OPCODE (assignment, res, lhs);
      token_after_newlines_must_be (TOK_COLON);
  ...
  switch (tok.type)
    {  
      case TOK_EQ:
        ...
        DUMP_OPCODE (assignment, lhs, rhs);
        break;
...
parse_variable_declaration (void)
{
  ...
  current_token_must_be (TOK_NAME);
  name = tok.data.uid;
  DUMP_OPCODE (decl_var, name);

  skip_newlines ();
  if (tok.type == TOK_EQ)
    {
      NEXT (expr, assignment_expression);
      DUMP_OPCODE (assignment, name, expr);
    }
  else
    lexer_save_token (tok);
}
